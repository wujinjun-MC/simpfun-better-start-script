...
- 可用的连接方式:
    - simpfun默认提供的sftp (纯SFTP服务)
        ...
        - 一般不支持文件"seek" (随机读取文件的一部分)
        - 文件兼容性差 (symlink无法读取/创建)
    - remotemode=1,2 创建的 sshd / ssld (带shell的"全功能"SFTP)
        - **消耗实例的每日上行流量限制**
        - 实例必须保持运行
        - 支持必须使用shell的功能，包括修改时间同步、校验和，节省上传流量
        - 正常的 Keepalive ，和大部分sshd的行为一致
        - 支持文件"seek"，提高大文件部分读取的效率
        - 文件兼容性好 (覆盖普通Linux的大部分文件，除 socket、block/char dev 等 edge cases)
